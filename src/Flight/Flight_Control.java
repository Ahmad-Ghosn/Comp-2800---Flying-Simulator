...
public class Flight_Control {
	...
	public Flight_Control(TransformGroup targetTG, TransformGroup followTG) {
	...
	private double getAccel(){	//Force to be applied forwards relative to the plane
		return this.throttle*20;	//Caps the acceleration at 50
	}
	...
	private double getLift(){	//Force to be applied upwards relative to the plane
		if (curSpeed > 0){
			return Math.log(Math.abs(this.curSpeed))*5;	//Gives a nice curve that counters gravity quickly, and doesn't get too high
		} else return 0.0d;
	}
	...
	public void integrateTransformChanges() {
		...
		offset.lookAt(
				new Point3d(this.vpPos.x + camLoc.x, this.vpPos.y + camLoc.y, this.vpPos.z + camLoc.z),
