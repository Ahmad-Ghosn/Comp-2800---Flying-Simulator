...
public class Display_Window extends JPanel implements MouseListener {
	...
	public static BranchGroup create_Scene_flight(){
		...
		sceneTG.addChild(Environment.createBG("sky"));
		TransformGroup planeTG = new TransformGroup();
		planeTG.setName("Plane");
		
		planeTG.addChild(new Import_Plane().position_object());
		Environment t = new Environment();
		Flight_Control_Behaviour control = new Flight_Control_Behaviour(planeTG, su.getViewingPlatform().getViewPlatformTransform(), (TransformGroup) t.position_object());
		control.setSchedulingBounds(new BoundingSphere(new Point3d(), 100000.0));
		
		sceneTG.addChild(t.position_object());
